# 梁平鑫系統 — 開發維護規範 v2（2026/07）

> 本文件是系統的**唯一權威參考**。新增功能、修 bug、效能調校前先讀相關章節。
> 取代舊版《梁平鑫系統開發規範.md》（2026/06），新增：Auth 借用機制、Service Worker、
> 紅點提醒契約、並行讀取規範、AI 協作工作流程。
...
| Functions | Node.js 20（→ 2026/10/30 前升 22），每日備份 `dailyFirestoreBackup` UTC 17:00 |
...
| 核心 JS | `auth-guard.js`（權限守衛+Auth 借用）、`sw.js`（Service Worker） |
| 人事 | `hr.html`、`batch_accounts.html` |
| 打卡 | `clock.html` + 子頁 `attendance / anomaly / anomaly_query / realtime / temp_clock` |
| 排班薪資 | `schedule.html`、`salary.html` |
| 回報 | `price.html`、`cctv_report.html`、`delivery_report.html`、`meeting.html` |
| 帳號權限 | `account.html`、`store_accounts.html`、`permissions.html`、`device_approval.html` |
| 系統 | `task.html`（店務追蹤）、`oplog.html`、`backup.html`、`restore.html`、`checkout.html` |
...
hr、clock、clock_attendance、clock_anomaly、clock_anomaly_query、clock_realtime、
clock_temp、schedule（含 employee）、price、device、meeting、salary、account、cctv、
...
| 打卡 | `attendance`、`attendance_anomaly`、`attendance_temp` |
...
`attendance` / `attendance_anomaly` / `attendance_temp` 每筆**必含**：
```javascript
{ date:'2026-06-27' /*YYYY-MM-DD 字串，查詢用*/, workLocation, empId, occurredAt /*ISO 排序用*/ }
```
> Flutter app 寫入端上線前必須對齊此契約，否則查詢頁抓不到資料。

回報類（price/cctv/delivery）需 `date`＋店名欄位（紅點提醒依賴，見 2-4）。
...
**維護規則：**
- 改 `auth-guard.js` 或想強制全站更新 → **bump `CACHE_NAME`**（`lp-xin-v3` → v4…），
  activate 會清舊快取強制重抓
...
| attendance.html / temp_clock.html 查詢優化 | 尚為全集合讀取，等寫入端 date 契約對齊後改 |
| Flutter 寫入端 date 欄位 | 對齊 4-3 契約後才可上線打卡三頁 |
| Flutter 完整測試 | 端對端 + iOS APNs 憑證 |
| Omada 外部存取 | port forwarding、移除 `online:true` bypass |
| Repo 遷移 | 移至 GitHub Org `Liangping-system`（低流量時段） |
| users 共用快取（選作） | auth-guard 加 loadAllUsers() sessionStorage 快取 |
